...
# ---------------------------
# REMOVE DATA QUALITY CHECKS
# ---------------------------
# No validation step here, we directly partition the data and write it out
# Add partition columns to the data
users_partitioned = add_partition(users_df)
transactions_partitioned = add_partition(transactions_df)
clickstream_partitioned = add_partition(clickstream_df)
# ---------------------------
# WRITE VALID DATA TO PROCESSED BUCKET
# ---------------------------
write_parquet(users_partitioned, f"s3://{PROCESSED_BUCKET}/user_sample/")
write_parquet(transactions_partitioned, f"s3://{PROCESSED_BUCKET}/transaction_sample/")
write_parquet(clickstream_partitioned, f"s3://{PROCESSED_BUCKET}/clickstream/")
# ---------------------------
# WRITE SUMMARY FILE TO S3
# ---------------------------
summary_data = [
    ("user_sample", users_df.count(), 0),  # 0 invalid count as we removed the check
    ("transaction_sample", transactions_df.count(), 0),
    ("clickstream", clickstream_df.count(), 0)
]
